...
public class sensorToDB extends HttpServlet {
	  ...
	  // This will load the driver and establish a connection
	    super.init(config);
		String user = "scialpiv";
	    String password = "pergbowT7";
...
// select data for this sensor, limit results to 1 record
	String selectSQL = "select SensorValue, TimeInserted from sensorUsage where "+
	                   "SensorName = '" + sensorname + 
	                   "' order by TimeInserted desc limit " + counter + ";";
  ...
  while (rs.next()) {
	  sensValue = rs.getString(1);
	  sensTime = rs.getString(2);
        	retrievedSensorData = rs.getString(1) + " " + rs.getString(2);
...
//  		      "\": \"" + sensorValueStr + "\"}}";
      ...
      String jsonOut = "{\"sensor\": {\"name\":\"" + sensorname + "\", \"value\":\"" + sensValue + "\", \"time\":\"" + sensTime + "\"" + "}}";
      System.out.println("JSON OUT: " + jsonOut);
